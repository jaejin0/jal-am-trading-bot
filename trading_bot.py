...
'''
class TradingBot:
    def __init__(self, market_observation_feature_dim, market_observation_time_range, action_dim, trader_state_dim, budget, threshold, transaction_fee, buffer_size, learning_rate):
        # model configuration
        self.model = JAL_AM(market_observation_feature_dim * market_observation_time_range, action_dim, trader_state_dim, learning_rate) 
    ...
    def action(self, market_observation, trader_state):
        action_prob = self.model.policy(market_observation, trader_state)
        print(action_prob)
        action = action_prob.max().indices.view(1, 1)
